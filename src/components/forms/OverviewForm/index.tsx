...
import { supabaseUpdateImg, supabaseUploadImg } from "@/lib/supabase";
  ...
  const onSubmit = async (val: z.infer<typeof overviewFormSchema>) => {
    try {
      ...
      if (typeof val.image === "object") {
        const uploadImg = await supabaseUploadImg(val.image, "company");
                    ...
                    >
                      ...
                      <SelectContent>
                        {LOCATION_OPTIONS.map((item: optionType, i: number) => (
                          <SelectItem value={item.id} key={item.id + i}>
                            {item.label}
                          </SelectItem>
                        ))}
                      </SelectContent>
